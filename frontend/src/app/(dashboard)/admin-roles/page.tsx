...
export default function AdminRolesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to load admin roles");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createRole = async () => {
    ...
    try {
      await api.adminRoles.create(token, {
      ...
      setError(e instanceof Error ? e.message : "Failed to create admin role");
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.adminRoles.update(token, editingId, {
        name: editForm.name,
      ...
      setError(e instanceof Error ? e.message : "Failed to save admin role");
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteRole = async (id: number) => {
    ...
    const ok = window.confirm("Delete this admin role?");
    if (!ok) return;
    ...
    try {
      await api.adminRoles.delete(token, id);
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to delete admin role");
    } finally {
      setSaving(false);
    }
  };
